...
And also with a whole lotta code (and feature inspiration) from:
	http://wlourf.deviantart.com/art/Text-widget-for-Conky-1-42-192574368
...
And with the box drawing functions almost verbatim from:
	http://wlourf.deviantart.com/art/Box-widget-for-conky-1-1-195130450
	http://u-scripts.blogspot.com/2011/01/box-widget_27.html

URLs are still working as of 21 May 2015. You never know, web pages and stuff. 
...
NOTE:
Until I could find a way to position text relative to the baseline
WITHOUT being shifted by the descent/ascent values of the text,
I'm disabling all text-related functions.

This weird text-drawing behavior's due to the way Cairo renders text.
It can theoretically be fixed by aligning strings character-by-character,
but individual horizontal spacing adjustments are difficult; I still
don't get how individual characters work in Cairo.

But until then, you can just add the text you need displayed in
the Conky config, set the font styles with "font", then move
them to the desired positions with "goto", "voffset", and "offset".
...
CHANGELOG:
- Disabled text-related operations.
- Added wlourf's box widget (which easily fits, since it also uses a table containing configs).
